...
public class FriendDaoImplSQLite extends SQLiteOpenHelper implements FriendDao {
    private static final String DB_NAME = "purnama.db";
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if(oldVersion < newVersion){
            String sql = "drop table if exists friend"; db.execSQL(sql);
            onCreate(db);
        }
    ...
    public void delete(int id) {
        String sql = "delete from friend where id=?"; getWritableDatabase().execSQL(sql, new Object[]{id});
    ...
    public Friend getAFriendById(int id) {
        ...
        if(cursor.moveToFirst()) result = new Friend(
